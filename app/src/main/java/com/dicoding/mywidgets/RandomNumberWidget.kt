...
class RandomNumberWidget : AppWidgetProvider() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (intent.action == WIDGET_CLICK) {
            ...
            //note remoteView
            // adalah komponen yang dapat kita gunakan untuk mengambil data layout dari widget yang kita pakai
            val views = RemoteViews(context.packageName, R.layout.random_number_widget)
            //note:
            //kode setTextViewText() di mana kita mengubah string text pada komponen R.id.appwidget_text.
